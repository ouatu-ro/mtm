...
def lowering_smoke_rows(fixture: TMFixture) -> list[list[object]]:
    ...
    assemble_instruction(builder, Halt(), state="start", continuation_label="NEXT")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=0, max_steps=5)
    rows.append(["HALT", result["status"], result["state"], result["head"], "-"])

    builder = TMBuilder(alphabet)
    assemble_instruction(builder, Goto("TARGET"), state="start", continuation_label="NEXT")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=0, max_steps=5)
    rows.append(["GOTO", result["status"], result["state"], result["head"], "-"])

    builder = TMBuilder(alphabet)
    assemble_instruction(builder, BranchCmp("EQ", "NEQ"), state="start", continuation_label="NEXT")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(CMP_FLAG), max_steps=5)
    rows.append(["BRANCH_CMP", result["status"], result["state"], result["head"], "cmp=0 -> NEQ"])

    builder = TMBuilder(alphabet)
    assemble_instruction(builder, BranchAt(RULE, "YES", "NO"), state="start", continuation_label="NEXT")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(RULE), max_steps=5)
    rows.append(["BRANCH_AT", result["status"], result["state"], result["head"], f"{RULE} -> YES"])

    builder = TMBuilder(alphabet)
    assemble_instruction(builder, WriteGlobal(CUR_SYMBOL, bits("01")), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(CUR_SYMBOL), max_steps=10)
    ...
    builder = TMBuilder(alphabet)
    assemble_instruction(builder, Seek(RULES, "L"), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=0, max_steps=300)
    rows.append(["SEEK", result["status"], result["state"], result["head"], f"at {RULES}"])

    builder = TMBuilder(alphabet)
    assemble_instruction(builder, SeekOneOf((RULE, END_RULES), "R"), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(RULES), max_steps=300)
    rows.append(["SEEK_ONE_OF", result["status"], result["state"], result["head"], f"at {RULE}"])

    builder = TMBuilder(alphabet)
    assemble_instruction(builder, FindFirstRule(), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=0, max_steps=300)
    rows.append(["FIND_FIRST_RULE", result["status"], result["state"], result["head"], f"at {RULE}"])

    builder = TMBuilder(alphabet)
    assemble_instruction(builder, FindNextRule(), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(RULE), max_steps=300)
    ...
    builder = TMBuilder(alphabet)
    assemble_instruction(builder, CopyLocalGlobal(WRITE, WRITE_SYMBOL, 2), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(RULE), max_steps=500)
    final_left_band, _ = split_runtime_tape(result["tape"])
    ...
    builder = TMBuilder(alphabet)
    assemble_instruction(builder, CompareGlobalLiteral(CUR_STATE, bits("10")), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(CUR_STATE), max_steps=500)
    final_left_band, _ = split_runtime_tape(result["tape"])
    ...
    builder = TMBuilder(alphabet)
    assemble_instruction(builder, CompareGlobalLocal(CUR_STATE, STATE, 2), state="start", continuation_label="DONE")
    result = run_raw_tm(builder.build("start"), runtime_tape, head=address_of(RULE), max_steps=1000)
    final_left_band, _ = split_runtime_tape(result["tape"])
    cmp_index = final_left_band.index(CMP_FLAG)
...
__all__ = ["assemble_instruction", "lowering_smoke_rows"]
